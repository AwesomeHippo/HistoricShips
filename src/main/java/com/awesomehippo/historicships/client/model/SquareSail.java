...
public final class SquareSail {
    ...
    public static final float GRID_OVERLAP = 0.48F;
    public static final float SOLID_MICRO = 0.04F;
    public static final float SOLID_THICK_MIN = 1.05F;
    public static final float SOLID_THICK_MUL = 1.55F;

    public static float solidCellThick(float thick) {
        return Math.max(thick, SOLID_THICK_MIN) * SOLID_THICK_MUL;
    }
    ...
    public static PartDefinition add(PartDefinition yardParent, String name, float yardY, float xOff, float height, float halfSpanH, float halfSpanF, float thick, int solidU, int solidV, int stripeU, int stripeV, int rigU, int rigV, CubeDeformation d0) {
        ...
        return addMapped(yardParent, name, yardY, xOff, height, halfSpanH, halfSpanF, thick, solidU, solidV, 0, 0, rigU, rigV, d0, true);
    }

    public static PartDefinition addMapped(PartDefinition yardParent, String name, float yardY, float xOff, float height, float halfSpanH, float halfSpanF, float thick, int solidU, int solidV, int atlasW, int atlasH, int rigU, int rigV, CubeDeformation d0, boolean addHead) {
        ...
        float cellThick = solidCellThick(thick);
        ...
        float yOverlap = rowH * GRID_OVERLAP;
        float hemY = -height - yOverlap;
        float footY = hemY - 0.50F;
        float tFoot = (ROWS - 0.5F) / (float) ROWS;
        float clothHalf = Mth.lerp(tFoot, halfSpanH, halfSpanF);
        float footOver = (clothHalf * 2.0F / (float) COLS) * GRID_OVERLAP;
        float boomHalf = clothHalf + footOver + 0.20F;
        sail.addOrReplaceChild("foot", CubeListBuilder.create().texOffs(rigU, rigV).addBox(sparX - sparD * 0.5F, footY - 0.42F, -boomHalf, sparD, 0.85F, boomHalf * 2.0F, d0), PartPose.ZERO);
        sail.addOrReplaceChild("clew_p", CubeListBuilder.create().texOffs(rigU, rigV).addBox(sparX - 0.22F, footY - 0.20F, -boomHalf + 0.02F, 0.55F, 0.55F, 0.55F, d0), PartPose.ZERO);
        sail.addOrReplaceChild("clew_s", CubeListBuilder.create().texOffs(rigU, rigV).addBox(sparX - 0.22F, footY - 0.20F, boomHalf - 0.57F, 0.55F, 0.55F, 0.55F, d0), PartPose.ZERO);
        return sail;
    }
    ...
    private static void generateSolidGrid(PartDefinition cloth, float height, float halfSpanHead, float halfSpanFoot, float thick, int solidU, int solidV, int atlasW, int atlasH, CubeDeformation d0) {
        ...
        for (int row = 0; row < ROWS; row++) {
            ...
            float zOverlap = colW * GRID_OVERLAP;
            ...
            for (int col = 0; col < COLS; col++) {
                ...
                int texU = faceU;
                int texV = faceV;
                float xBias = (((col + row) & 1) == 0) ? -SOLID_MICRO : SOLID_MICRO;
