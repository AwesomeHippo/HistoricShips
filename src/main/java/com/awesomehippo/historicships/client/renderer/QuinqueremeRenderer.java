...
public class QuinqueremeRenderer extends EntityRenderer<QuinqueremeEntity, OarShipRenderState> {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(NapoleonShipMod.MODID, "textures/entity/quinquereme.png");
    ...
    public void submit(OarShipRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        poseStack.pushPose();
        ShipRenderPose.apply(poseStack, state.ageInTicks, state.yRot, SCALE, state.localPassenger, ShipRenderPose.QUINQUEREME);
